...
type WindowEthereumPhantom = Window["phantom"]["ethereum"] & {
  isPhantom?: boolean;
...
export function getPhantomWalletInjectedProvider(): Window["phantom"]["ethereum"] {
  const isPhantomWallet = (ethereum?: WindowEthereumPhantom) => {
    // Identify if Phantom Wallet injected provider is present.
    const isInjected = !!ethereum?.isPhantom;
  ...
  const ethereum = window.phantom.ethereum as WindowEthereumPhantom;

  // Phantom Wallet was injected into window.phantom.ethereum.
...
export class PhantomConnector extends AbstractConnector {
